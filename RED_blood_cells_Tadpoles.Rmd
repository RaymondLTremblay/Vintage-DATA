...
Note here that Pearson did not show the whole distribution on the values, which gives an impression of the slope being more negative (although it is the same).  In addition he calculated the mean of corpuscule length for different values of body length.   In addition the 95% CI of the regression line is not present.  

![Regression ](Graficos/Tadpole_Regression_Diagram_1.png)

We can get a similar type of figure 
